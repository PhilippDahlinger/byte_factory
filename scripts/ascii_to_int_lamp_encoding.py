...
class ASCIIGUI:
    def __init__(self):
        ...
        # current code var
        self.code_var = tk.IntVar(self.root)
        self.code_var.set(32)  # Set default to space
        self.code_var.trace_add("write", self.refresh_button_colors)

        # Label to show the current character being modified
        self.char_label = tk.Label(self.root, text=f"Current Code: {self.code_var.get()}, {chr(self.code_var.get())}")
        ...
        self.frame.pack()
        self.buttons = self.create_character_grid(self.code_var.get())
        ...
        prev_button = tk.Button(self.root, text="Prev. Character", command=self.prev_char)
        prev_button.pack()

        # Entry field for direct character code input
        self.entry_var = tk.StringVar(self.root)
        self.entry = tk.Entry(self.root, textvariable=self.entry_var)
        self.entry.pack()
        self.entry.bind("<Return>", self.update_code_var_from_entry)

        self.root.mainloop()

    def update_code_var_from_entry(self, event):
        try:
            new_code = int(self.entry_var.get())
            if 32 <= new_code <= 255:  # Ensure it's within the valid range
                self.code_var.set(new_code)
                self.refresh_button_colors()
                self.update_char_label()
        except ValueError:
    ...
    def next_char(self):
        ...
        next_char_code = current_code + 1
        if next_char_code > 255:  # Loop back to the start if beyond 255
            next_char_code = 32  # Start from space again
        self.code_var.set(next_char_code)
        self.refresh_button_colors()
        self.update_char_label()
    ...
    def load_encodings(self, file=os.path.join("output", "font_encodings_2.json")):
        try:
            ...
            result = {i: self.create_empty_grid() for i in
                    range(32, 255)}  # Create a default for all printable ASCII chars
            # migrate from old font_encodings
            try:
                with open(os.path.join("output", "font_encodings.json"), "r") as f:
                    old_result = json.load(f)
            except (FileNotFoundError, json.JSONDecodeError):
                return result
            for char_idx in range(32, 128):
                if chr(char_idx) in old_result.keys():
                    result[char_idx] = old_result[chr(char_idx)]
            return result


    # Save font encodings to a JSON file
    def save_encodings(self, encodings, file=os.path.join("output", "font_encodings_2.json")):
    ...
    def toggle_button(self, row, col):
        ...
        self.encodings[code][row][col] = not self.encodings[code][row][col]
        self.refresh_button_colors()
    ...
    def update_char_label(self):
        code = self.code_var.get()
        self.char_label.config(text=f"Current Code: {code}, {chr(code)}")


if __name__ == "__main__":
    # Font "inspired" by https://fontstruct.com/fontstructions/show/2066750/classic-5x7-2
    ASCIIGUI()
    ...
    with open("output/font_encodings_2.json", "r") as f:
